...
To install the chart with the release name `choerodon-gateway`:

```console
$ helm repo add c7n https://openchart.choerodon.com.cn/choerodon/c7n
$ helm repo update
$ helm install choerodon-gateway c7n/choerodon-gateway
```
...
| containerPort.actuatorPort | int | `8081` | server management port |
...
| extraEnv.HZERO_GATEWAY_AUTHENTICATE_REDIS | bool | `true` |  |
| extraEnv.HZERO_GATEWAY_FILTER_COLLECT_SPAN_ENABLED | bool | `true` |  |
| extraEnv.HZERO_GATEWAY_FILTER_COMMON_REQUEST_CHECK_ORGANIZATION | bool | `true` |  |
| extraEnv.HZERO_GATEWAY_FILTER_COMMON_REQUEST_CHECK_PROJECT | bool | `true` |  |
| extraEnv.HZERO_GATEWAY_FILTER_COMMON_REQUEST_ENABLE | bool | `true` |  |
| extraEnv.HZERO_GATEWAY_HELPER_ENABLE_JWT_LOG | bool | `false` |  |
| extraEnv.HZERO_REQUEST_ENCRYPT_MODE | string | `"encrypt"` |  |
| extraEnv.JAVA_OPTS | string | `"-XX:MaxRAMPercentage=90.0"` |  |
| extraEnv.LOG_LEVEL | string | `"info"` |  |
| extraEnv.REACTOR_NETTY_IOSELECTCOUNT | int | `6` |  |
| extraEnv.REACTOR_NETTY_IOWORKERCOUNT | int | `12` |  |
| extraEnv.REACTOR_NETTY_NATIVE | bool | `false` |  |
| extraEnv.REACTOR_NETTY_POOL_MAXCONNECTIONS | int | `100` |  |
| extraEnv.SPRING_CLOUD_CONFIG_ENABLED | bool | `false` |  |
| extraEnv.SPRING_CLOUD_CONFIG_URI | string | `"http://register-server:8000"` |  |
| extraEnv.SPRING_CLOUD_LB_CACHE_TTL | string | `"10s"` |  |
| extraEnv.SPRING_CLOUD_LB_RETRY_ENABLED | bool | `false` |  |
| extraEnv.SPRING_CLOUD_LB_RETRY_NEXT | int | `1` |  |
| extraEnv.SPRING_CLOUD_LB_RETRY_SAME | int | `0` |  |
| extraEnv.SPRING_DATASOURCE_PASSWORD | string | `"password"` | Datasource password |
| extraEnv.SPRING_DATASOURCE_URL | string | `"jdbc:mysql://localhost:3306/hzero_platform?useUnicode=true&characterEncoding=utf-8&useSSL=false&useInformationSchema=true&remarks=true&serverTimezone=Asia/Shanghai"` | Datasource url |
| extraEnv.SPRING_DATASOURCE_USERNAME | string | `"choerodon"` | Datasource username |
| extraEnv.SPRING_REDIS_CONNECT_TIMEOUT | int | `2000` |  |
| extraEnv.SPRING_REDIS_DATABASE | int | `4` | Redis database |
| extraEnv.SPRING_REDIS_HOST | string | `"localhost"` | Redis host |
| extraEnv.SPRING_REDIS_PORT | int | `6379` | Redis port |
| extraEnv.SPRING_REDIS_READ_TIMEOUT | int | `5000` |  |
...
| image.repository | string | `"c7n/server"` | service image repository |
...
| service.nodePort | object | `{"actuator":30095,"server":30094}` | Specify the nodePort value for the LoadBalancer and NodePort service types. |
| service.port | object | `{"actuator":8081,"server":8080}` | server Service port |
