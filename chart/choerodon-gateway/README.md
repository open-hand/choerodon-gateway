...
```bash
$ helm install c7n/choerodon-gateway --name choerodon-gateway
```
...
`image.repository` | 镜像库地址 | `registry.choerodon.com.cn/choerodon/choerodon-gateway`
...
`service.name` | service名称 | `choerodon-gateway`
...
```bash
$ helm install c7n/choerodon-gateway \
    --set env.open.SKYWALKING_OPTS="-javaagent:/agent/skywalking-agent.jar -Dskywalking.agent.application_code=choerodon-gateway  -Dskywalking.agent.sample_n_per_3_secs=-1 -Dskywalking.collector.backend_service=oap.skywalking:11800" \
